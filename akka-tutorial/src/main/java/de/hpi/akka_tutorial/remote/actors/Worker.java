		...
		for (long i = message.rangeMin; i <= message.rangeMax; i++) {
			if (isPrime(i)) {
				...
				if (primeBuffer.size() >= MAX_PRIMES_PER_MESSAGE) {
					...
					// Create a copy of the elements in the buffer before sending them;
					// never send mutable objects in a message!!!
					ArrayList<Long> primeBufferCopy = new ArrayList<>(primeBuffer);
					
					// Send the intermediate results to the master actor
					this.getSender().tell(
							new Master.PrimesMessage(message.id, primeBufferCopy, false), this.getSelf());
