...
/**
 * The master receives ranges of numbers that it should find all primes in.
 * This is done by delegation to slaves.
 */
public class Master extends AbstractLoggingActor {
	...
	private boolean hasFinished() {
		// The master has finished if
		// (1) there will be no further requests and
		// (2) either all requests have been processed or there are no more workers to process these requests
		return !this.isAcceptingRequests
				&& (!this.schedulingStrategy.hasTasksInProgress() || this.schedulingStrategy.countWorkers() < 1);
	}
